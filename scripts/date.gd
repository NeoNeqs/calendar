...
const TIMEZONE_OFFSETS_FROM_UTC: Dictionary[String, int] = {
	"Africa/Abidjan": 0,
	"Africa/Accra": 0,
...
@export var timezone: StringName
@export var unix_timestamp: int


func to_unix_date() -> int:
	return unix_timestamp - (unix_timestamp % 86400)


func to_human_date() -> String:
	var local_timestamp: int = unix_timestamp + TIMEZONE_OFFSETS_FROM_UTC[timezone] * 60
	var date: Dictionary = Time.get_date_dict_from_unix_time(local_timestamp)
	
	return "%s %02d %s %d" % [
			WEEKDAY_NAMES[date.weekday - 1], date.day, 
			MONTH_NAMES[date.month - 1], date.year
	]

func to_human_time() -> String:
	var local_timestamp: int = unix_timestamp + TIMEZONE_OFFSETS_FROM_UTC[timezone] * 60
	var seconds_into_day: int = local_timestamp % 86400
	var hours := int(seconds_into_day / 3600.0)
	var minutes := int((seconds_into_day % 3600) / 60.0)
	
	return "%02d:%02d" % [hours, minutes]

static func utc_now(hours: int = 0, minutes: int = 0, seconds: int = 0) -> Date:
	var u := int(Time.get_unix_time_from_system())
	
	u += seconds + 60 * minutes + 3600 * hours
	var date := Date.new()
	date.unix_timestamp = u
	date.timezone = "UTC"
	
	return date

static func now(hours: int = 0, minutes: int = 0, seconds: int = 0) -> Date:
	var u := int(Time.get_unix_time_from_system())
	
	u += seconds + 60 * minutes + 3600 * hours
	var date := Date.new()
	date.unix_timestamp = u
	date.timezone = Time.get_time_zone_from_system().name
	
	return date
	

# TZID=Europe/Warsaw;VALUE=DATE-TIME:20251001T094500
static func from_ical_date_string(date_string: String) -> Date:
	var date := Date.new()
	date.timezone = Date.__parse_timezone(date_string)
	date.unix_timestamp = Date.__parse_timestamp(date_string, date.timezone)
	return date

static func __parse_timezone(date_string: String) -> String:
	var offset := ICalParser.ICAL_EVENT_TZID.length()
	if date_string[-1] == 'Z':
		return "UTC"
	
	var semicolon_index := date_string.find(';', offset)
	
	return StringName(date_string.substr(offset, semicolon_index - offset))

static func __parse_timestamp(date_string: String, _timezone: StringName) -> int:
	var offset: int = date_string.rfind(':', date_string.length() - 15) + 1
	var p: String = (
		date_string.substr(offset, 4) + '-' + 
		date_string.substr(offset + 4, 2) + '-' +
		date_string.substr(offset + 6, 2) + 'T' +
		date_string.substr(offset + 9, 2) + ':' +
		date_string.substr(offset + 11, 2) + ':' +
		date_string.substr(offset + 13, 2)
	)
	
	var unix: int = Time.get_unix_time_from_datetime_string(p)
	
	return unix - TIMEZONE_OFFSETS_FROM_UTC[_timezone] * 60
